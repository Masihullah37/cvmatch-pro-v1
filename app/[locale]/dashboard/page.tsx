        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <div className="bg-white/80 backdrop-blur-xl border border-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50 flex flex-col justify-between group hover:border-primary/20 transition-all">
            <div className="flex justify-between items-start mb-6">
              ...
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Crédits</span>
            </div>
            <div>
              <div className="text-5xl font-black text-slate-900 tracking-tighter mb-2">{credits}</div>
              <p className="text-slate-500 text-sm font-bold">Générations restantes</p>
            </div>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-xl border border-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50 flex flex-col justify-between group hover:border-accent/20 transition-all">
            <div className="flex justify-between items-start mb-6">
              ...
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Meilleur Score</span>
            </div>
            <div>
              <div className="text-5xl font-black text-slate-900 tracking-tighter mb-2">
                {analyses.length > 0 ? Math.max(...analyses.map(a => a.atsScore || 0)) : 0}%
              </div>
              <p className="text-slate-500 text-sm font-bold">Compatibilité Maximale</p>
            </div>
          </div>
          ...
          <div className="bg-slate-900 p-8 rounded-[2.5rem] shadow-2xl shadow-slate-900/20 flex flex-col justify-between text-white group relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="text-3xl font-black tracking-tight mb-2 uppercase italic">
                {dbUser?.plan === 'monthly' ? 'Abonné PRO' : dbUser?.plan === 'one_time' ? 'Plan Starter' : 'Utilisateur Free'}
              </div>
              <Link href="/#pricing" className="text-primary text-xs font-black uppercase tracking-widest flex items-center gap-2 hover:translate-x-1 transition-transform">
                {dbUser?.plan === 'free' ? 'Passer en PRO' : 'Gérer mon plan'} <ArrowRight size={14} />
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="mb-12">
          ...
          {generatedCvs.length === 0 ? (
            <div className="bg-white border border-slate-100 rounded-[2.5rem] p-12 text-center shadow-sm">
              <p className="text-slate-400 font-bold">Vous n'avez pas encore généré de CV.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {generatedCvs.map((cv) => (
                <div key={cv.id} className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all group">
                  <div className="flex items-center gap-4 mb-6">
                    <div className="w-12 h-12 bg-indigo-50 rounded-xl flex items-center justify-center">
                      <FileText className="text-indigo-500" />
                    </div>
                    <div>
                      <h4 className="font-black text-slate-900 text-sm">{cv.templateStyle}</h4>
                      <p className="text-[10px] font-bold text-slate-400 uppercase">{new Date(cv.createdAt).toLocaleDateString()}</p>
                    </div>
                  </div>
        ...
        <div className="space-y-8">
          ...
          {analyses.length === 0 ? (
            <div className="bg-white/50 backdrop-blur-md border-2 border-dashed border-slate-200 rounded-[3rem] p-16 text-center">
              ...
              <h3 className="text-2xl font-black text-slate-900 mb-2">Pas encore d'analyses</h3>
              <p className="text-slate-500 font-medium mb-8 max-w-sm mx-auto">Analysez votre premier CV pour voir vos scores et générer des modèles professionnels.</p>
              <Link href="/" className="bg-primary text-white px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest inline-block shadow-lg shadow-primary/20">
            ...
            <div className="space-y-4">
              {analyses.map((analysis) => (
                <div key={analysis.id} className="bg-white hover:bg-slate-50/50 border border-slate-100 rounded-[2rem] p-6 transition-all flex flex-col md:flex-row md:items-center justify-between gap-6 group shadow-sm hover:shadow-xl hover:shadow-slate-200/40">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <h4 className="font-black text-slate-900 text-lg group-hover:text-primary transition-colors line-clamp-1">{analysis.jobTitle || analysis.jobUrl || 'Analyse CV'}</h4>
                      <div className="flex items-center gap-4 mt-1">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                           <Clock size={12} /> {new Date(analysis.createdAt!).toLocaleDateString('fr-FR')}
                        </span>
                      </div>
                    </div>
                  </div>
