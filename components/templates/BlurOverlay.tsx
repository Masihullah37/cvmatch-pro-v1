...
export default function BlurOverlay({ isPaid, analysisId }: BlurOverlayProps) {
  ...
  const handlePayment = async (type: 'one_time' | 'subscription') => {
    ...
    router.push(`/checkout/${type === 'one_time' ? 'one-time' : 'subscription'}?analysisId=${analysisId}`);
  };
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center backdrop-blur-md bg-background/40">
          <div className="bg-card border border-border/50 p-8 rounded-2xl shadow-2xl max-w-xl w-full mx-4 relative overflow-hidden">
            ...
            <div className="text-center mb-8">
              ...
              <h2 className="text-2xl font-bold mb-2">Débloquez vos CV optimisés</h2>
              <p className="text-muted-foreground">Sélectionnez une option pour télécharger vos modèles générés par IA et parfaitement adaptés à l'ATS.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                <Download className="mb-3 text-muted-foreground group-hover:text-primary transition-colors" size={28} />
                <span className="font-bold text-lg">3€</span>
                <span className="text-sm font-medium">Paiement unique</span>
                <span className="text-xs text-muted-foreground mt-2 text-center">Téléchargez un seul modèle au choix</span>
              </button>
              ...
              >
                ...
                <span className="font-bold text-lg text-primary">13€ / mois</span>
                <span className="text-sm font-medium">Abonnement pro</span>
                <span className="text-xs text-muted-foreground mt-2 text-center">Accès illimité (10 CV / mois)</span>
              </button>
            </div>
